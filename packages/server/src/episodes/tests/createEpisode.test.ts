...
import createEpisode from '@src/episodes/createEpisode';
import CreateEpisodeDto from '@src/episodes/CreateEpisodeDto';

describe('createEpisode', (): void => {
  ...
  it('should create a new podcast', async (): Promise<void> => {
    ...
    await createEpisode(createEpisodeDto);
  ...
  it('should throw an error when validation is not passed', async () => {
    ...
    // @ts-ignore
    await expect(createEpisode(episodeObj)).rejects.toThrowError();
  });
});
